...
public class CultivationScreen extends Screen {
    ...
    public CultivationScreen() {
        super(Component.translatable("screen.earth_online_xuanhuan.cultivation.title"));
    }
    ...
    public void tick() {
        ...
        if (++refreshTicks >= 20) {
            refreshTicks = 0;
            EarthOnlineXuanhuanClient.requestCultivationRefresh();
        }
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        Component support = Component.translatable(status.seated()
                ? "screen.earth_online_xuanhuan.cultivation.support.cushion"
                : "screen.earth_online_xuanhuan.cultivation.support.free");
        ...
        if (h >= 194 && contentW >= 188) {
            int infoY = stageY - 12;
            Component source = Component.translatable("screen.earth_online_xuanhuan.cultivation.source",
    ...
    public boolean keyPressed(KeyEvent event) {
        if ((event.key() == InputConstants.KEY_LSHIFT || event.key() == InputConstants.KEY_RSHIFT)
                && EarthOnlineXuanhuanClient.cultivationStatus().seated()) {
            leaveCushion();
            return true;
        }
    ...
    private void performPrimaryAction() {
        if (EarthOnlineXuanhuanClient.cultivationStatus().seated()) {
            leaveCushion();
        } else {
            EarthOnlineXuanhuanClient.requestPractice();
        }
    }

    private void leaveCushion() {
        EarthOnlineXuanhuanClient.requestStopCultivating();
        onClose();
    }
    ...
    private void updateButtons() {
        ...
        for (Map.Entry<CultivationFocus, Button> entry : focusButtons.entrySet()) {
            ...
            entry.getValue().setTooltip(Tooltip.create(Component.translatable(unlocked
                    ...
                    : "screen.earth_online_xuanhuan.cultivation.focus.locked")));
        }
    ...
    private void drawStageTrack(GuiGraphicsExtractor g, int x, int y, int width, CultivationStatusPayload status) {
        ...
        Component remaining = Component.translatable("screen.earth_online_xuanhuan.cultivation.remaining",
