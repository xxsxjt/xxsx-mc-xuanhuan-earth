...
import com.xxsx.xuanhuanearth.XuanhuanJourney;
...
public class XuanhuanHandbookScreen extends Screen {
    ...
    public XuanhuanHandbookScreen() {
        super(Component.translatable("screen.xuanhuan_earth.handbook.title"));
    }
    ...
    protected void init() {
        ...
        prevButton = addRenderableWidget(Button.builder(Component.translatable("screen.xuanhuan_earth.handbook.prev"), b -> setPage(page - 1))
                .bounds(left + bookWidth() - 182, bottom, 76, 20).build());
        nextButton = addRenderableWidget(Button.builder(Component.translatable("screen.xuanhuan_earth.handbook.next"), b -> setPage(page + 1))
                .bounds(left + bookWidth() - 100, bottom, 76, 20).build());
        addRenderableWidget(Button.builder(Component.translatable("screen.xuanhuan_earth.handbook.close"), b -> onClose())
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float delta) {
        ...
        String bookTitle = tr("screen.xuanhuan_earth.handbook.title");
        g.text(font, bookTitle, left + (bw - font.width(bookTitle)) / 2, top + 8, INK, false);
        g.text(font, tr("screen.xuanhuan_earth.handbook.subtitle"), left + 14, top + 24, MUTED, false);
        g.text(font, tr("screen.xuanhuan_earth.handbook.page", page + 1, pages.size()), left + bw - 70, top + 24, MUTED, false);
    ...
    private List<Entry> liveStatusEntries() {
        ...
        entries.add(new Entry(Component.translatable(
                "screen.xuanhuan_earth.handbook.journey.progress",
        ...
        if (nextMilestone == null) {
            entries.add(new Entry(Component.translatable(
                    "screen.xuanhuan_earth.handbook.journey.complete").getString(), 10, GOLD));
        } else {
            entries.add(new Entry(Component.translatable(
                    "screen.xuanhuan_earth.handbook.journey.next",
        ...
        if (!status.isUnlocked(CultivationFocus.CIRCULATION)) {
            entries.add(new Entry(Component.translatable(
                    "screen.xuanhuan_earth.handbook.status.locked").getString(), 10, PURPLE));
        ...
        String nextKey = unlocked < CultivationFocus.values().length
                ? "screen.xuanhuan_earth.handbook.status.next.unlock"
                : "screen.xuanhuan_earth.handbook.status.next.train";
        entries.add(new Entry(Component.translatable(
                "screen.xuanhuan_earth.handbook.status.current",
    ...
    private static List<Page> createPages() {
        ...
        boolean magicLoaded = ModList.get().isLoaded("fantasy_earth")
                || ModList.get().isLoaded("earth_online_magic");
        if (!Minecraft.getInstance().getLanguageManager().getSelected().toLowerCase(Locale.ROOT).startsWith("zh")) {
            return List.of(
                    ...
                    page("Links", "5. Optional integrations", GOLD,
                            ...
                            magicLoaded
                                    ? "Fantasy Earth connected: qi and magic contributions are added into one shared mana pool."
                                    : "Fantasy Earth not installed: this cultivation route remains fully standalone."));
        }
        return List.of(
